...
# --- FUNÇÕES DE LIMPEZA (SÓ FUNCIONAM SE O REQUIREMENTS ESTIVER CERTO) ---
def faxina_tel(v):
    n = re.sub(r'\D', '', str(v))
...
if not st.session_state.logado:
            ...
            for i, r in df.iterrows():
                with st.expander(f"👤 {r.get('nome','-')} | 📅 {r.get('nascimento','-')}"):
                    ...
                    with c1:
                        ...
                        st.write(f"🌳 **Relacionado a:** {r.get('ascendente','-')}")
    ...
    with t3:
        ...
        with st.form("f_novo", clear_on_submit=True):
            ...
            with col1:
                ...
                f_vinc = st.radio("Vínculo:", ["Filho(a) de", "Cônjuge de"], horizontal=True)
            with col2:
                f_a = st.selectbox("Pessoa de Referência", ["Raiz"] + lista_nomes)
            ...
            if st.form_submit_button("SALVAR"):
                if f_n:
                    relacao = f"{f_vinc} {f_a}" if f_a != "Raiz" else "Raiz"
                    conj_val = f_a if "Cônjuge" in f_vinc else ""
                    # Ordem: Nome, Nascimento, Ascendente, Telefone, Email, Rua, Num, Conjuge, Bairro, CEP
                    dados = [f_n, faxina_data(f_d), relacao, faxina_tel(f_t), "", f_r, f_u, conj_val, f_b, ""]
    ...
    with t4:
        ...
        sel = st.selectbox("Selecione um membro", ["--"] + lista_nomes)
        if sel != "--":
