...
if not st.session_state.logado:
    ...
    if df_todo.empty: st.error("⚠️ Planilha não carregada.")
        ...
        with tabs[0]: # 1. Membros (Limpo!)
            sel_ids = []; c_top = st.container()
            for i, r in df_m.iterrows():
                c_sel, c_exp = st.columns([0.15, 3.85])
                if c_sel.checkbox("", key=f"s_{i}"): sel_ids.append(i)
                with c_exp.expander(f"👤 {r['nome']} | 🎂 {r.get('nascimento','-')}"):
            ...
            if sel_ids: c_top.download_button("📥 PDF SELECIONADOS", gerar_pdf_membros(df_m.loc[sel_ids]), "membros.pdf")

        with tabs[2]: # 3. Mural (Com Botão Limpar)
        ...
        with tabs[4]: # 5. Gerenciar
            ...
            if esc != "--":
                ...
                with st.form("g_f"):
                    ...
                    with g2: tv = st.radio("Vínculo", ["Filho(a) de", "Cônjuge de"]); rv = st.selectbox("Ref", ["Raiz"] + nomes_lista)
                    b1, b2 = st.columns(2)
                    if b1.form_submit_button("💾 ATUALIZAR"):
                        v_f = f"{tv} {rv}" if rv != "Raiz" else "Raiz"
                        requests.post(WEBAPP_URL, json={"action":"edit", "row":idx, "data":[esc, ed, v_f, et, "", "", "", "", "", ""]}); st.rerun()
                    if b2.form_submit_button("🗑️ EXCLUIR"): requests.post(WEBAPP_URL, json={"action":"edit", "row":idx, "data":[""]*10}); st.rerun()

        with tabs[5]: # 6. Árvore (Cônjuges Amarelos)
