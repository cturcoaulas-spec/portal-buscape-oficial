...
# LINKS OFICIAIS (ID: ...Zio e Versão 2 do Script)
...
if not st.session_state.logado:
    ...
    if df_todo.empty:
        st.warning("⚠️ Planilha vazia. Verifique se ela está 'Publicada na Web'.")
    else:
        ...
        with tabs[4]: # 5. Gerenciar
            ...
            if esc != "--":
                ...
                with st.form("g_f"):
                    ...
                    with g2: tv = st.radio("Vínculo", ["Filho(a) de", "Cônjuge de"]); rv = st.selectbox("Referência", ["Raiz"] + nomes_lista)
                    b1, b2 = st.columns(2)
                    if b1.form_submit_button("💾 ATUALIZAR"):
                        vf = f"{tv} {rv}" if rv != "Raiz" else "Raiz"
                        requests.post(WEBAPP_URL, json={"action":"edit", "row":idx, "data":[esc, ed, vf, et, "", "", "", "", "", ""]}); st.rerun()
