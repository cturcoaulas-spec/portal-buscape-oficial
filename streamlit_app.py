...
def aplicar_mascara_tel(v):
    ...
    return v # Retorna o que foi digitado se não tiver 10 ou 11 dígitos
...
if not st.session_state.logado:
    ...
    def carregar():
        try:
            ...
            df = df[df['nome'].str.strip() != ""] # Remove excluídos
            return df
    ...
    st.title("🌳 Portal Família Buscapé")
    # Uso de ícones fixos para evitar erro de tradução
    ...
    with t1:
        ...
        if not df.empty:
            for i, r in df.iterrows():
                ...
                with c_exp:
                    # Accordion: Streamlit abre vários, mas o filtro ajuda a focar
        ...
        with st.form("f_novo", clear_on_submit=True):
            ...
            if st.form_submit_button("💾 SALVAR"):
                if f_n in lista_nomes: st.error("⚠️ Já tem cadastro!")
                    ...
                    # Mascara aplicada no momento de salvar
    ...
    with t4:
        ...
        opcoes = ["Selecione um nome..."] + lista_nomes
        s_m = st.selectbox("Escolha quem deseja alterar", opcoes)
        
        if s_m != "Selecione um nome...":
